% IMBG=SM_bg_uniform_repeated_decay(t,bgOpt,cameraOpt)
% 
...
% cameraOpt is also added. In this version, multiple reference times t0 can
% be used, and eq 1 is then applied to the most recent reference time only.
% If t<t0 for all entries in t0, only steady state background is generated.
%
% input
...
%             distribution with mean value bgOpt.photons_per_pixel, plus
%             the appropriate EMCCD noise and offset.
...
function IMBG=SM_bg_uniform_repeated_decay(t,bgOpt,cameraOpt)
...
% find the most recently axtivated component of t0
tRef=max(t0(t0<=t));
%disp(num2str([tRef (t-tRef)/tD]))
if(~isempty(tRef))
    % generate the shotnoise background
    IMBG=poissrnd(IMones*Ess+Edec*exp(-(t-tRef)/tD));
else
    IMBG=poissrnd(IMones*Ess);    
end
